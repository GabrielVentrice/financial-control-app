  ...
  <Sidemenu>
    <div class="bg-background-page text-text-primary min-h-screen">
      ...
      <header class="h-[72px] px-10 flex items-center justify-between border-b border-border-base">
        <div>
          <h1 class="text-[22px] font-medium">Gastos por Categoria</h1>
          <p class="text-[13px] text-text-secondary mt-0.5">Análise de despesas organizadas por categoria</p>
        </div>
        <button
          @click="refreshData"
          :disabled="loading"
      ...
      <main class="max-w-[1280px] px-10 py-8 space-y-12">
        ...
        <div v-else-if="error" class="border-l-[3px] border-accent-danger bg-background-card p-5 rounded-lg">
          <h4 class="text-text-primary font-semibold text-[15px]">Erro ao carregar dados</h4>
          <p class="text-text-secondary text-[13px] mt-1">{{ error }}</p>
        </div>
        ...
        <template v-else>
          ...
          <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-background-card border border-border-base rounded-lg px-6 py-5 space-y-3">
              ...
              <p class="text-text-muted text-[13px]">Gastos não recorrentes</p>
            </div>
            ...
            <div class="bg-background-card border border-border-base rounded-lg px-6 py-5 space-y-3">
              ...
              <p class="text-text-muted text-[13px]">{{ totalTransactions }} transações</p>
            </div>
          </section>
          ...
          <section class="bg-background-card border border-border-base rounded-lg overflow-hidden">
            ...
            <div class="divide-y divide-border-base">
              <template v-for="category in categories" :key="category.name">
                ...
                >
                  <div class="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div class="col-span-5 flex items-center gap-3">
                      ...
                      <div>
                        <p class="text-[15px] font-medium text-text-primary">{{ category.name }}</p>
                        <p class="text-[13px] text-text-muted">Clique para expandir</p>
                      </div>
                    </div>

                    <!-- Transaction Count -->
                    <div class="col-span-2">
                      <span class="inline-block px-3 py-1 bg-background-section text-accent-info text-[13px] font-semibold rounded-md border border-border-base">
                        {{ category.count }}
                      </span>
                    </div>
                    ...
                    <!-- Total Value -->
                    <div class="col-span-2">
                      <p class="text-[16px] font-semibold text-text-primary">
                    ...
                    <div class="col-span-2">
                      <div class="space-y-1">
                        ...
                        <span class="text-[13px] font-medium text-text-secondary">{{ category.percentage.toFixed(1) }}%</span>
                      </div>
                    </div>
                ...
                <div v-if="expandedCategory === category.name" class="bg-background-section">
                  <div class="px-6 py-4">
                    <h4 class="text-[15px] font-medium text-text-primary mb-4">
                      ...
                      <div class="space-y-3">
                        ...
                        >
                          <div class="flex-1 space-y-1">
                            <p class="text-[15px] text-text-primary">{{ transaction.description }}</p>
                          ...
                          <p class="text-[15px] font-semibold text-accent-primary ml-4">
            ...
            <div v-if="categories.length === 0" class="text-center py-12">
              <p class="text-text-secondary text-[15px]">Nenhuma transação encontrada para o período selecionado</p>
            </div>
          </section>
        </template>
      </main>
